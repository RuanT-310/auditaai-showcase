...
@Component({
  ...
  standalone: true,
  template: `
    <header class="bg-g-bg-d text-g-r-bg-d py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
        
        <h1 class="text-5xl sm:text-7xl font-extrabold tracking-tight mb-2">
            Audit.<span class="text-g-alert">AI</span>
            </h1>
        
        <p class="text-g-int-d text-xl sm:text-2xl font-light mb-8">
        ...
        <div class="flex flex-wrap gap-4 sm:gap-6 mt-6">
            @for (award of awards(); track award.event) {
                <div class="flex items-center space-x-2 p-3 rounded-lg shadow-lg {{ award.color }} text-g-r-bg transition duration-300 hover:scale-[1.02]">
